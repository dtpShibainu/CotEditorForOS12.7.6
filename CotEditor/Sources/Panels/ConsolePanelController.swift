//
//  Console.swift (Background version)
//
//  CotEditor
//
...
// MARK: - Backgroud Console Controller (no UI)

final class ConsolePanelController {
    
    static let shared = ConsolePanelController()
    
    /// 内部ログ保持
    private(set) var logs: [Console.Log] = []
    
    private init() {}
    
    /// ログを追加（UIなし・バックグラウンドで動作）
    ...
    /// ログ内容をまとめて取得
    func allLogsText(options: Console.DisplayOptions = .all) -> String {
        return logs.map { log in
            var components: [String] = []
            if options.contains(.timestamp) {
                let dateString = DateFormatter.localizedString(from: log.date, dateStyle: .short, timeStyle: .medium)
                components.append("[\(dateString)]")
            }
            if options.contains(.title), let title = log.title {
